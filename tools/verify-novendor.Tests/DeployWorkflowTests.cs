...
public class DeployWorkflowTests
{
    ...
    [Fact]
    public void PublishWorkflow_RatchetsCoverageVariablesWithoutBlockingPublish()
    {
        var repoRoot = Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", ".."));
        var workflowPath = Path.Combine(repoRoot, ".github", "workflows", "deploy.yml");
        ...
        Assert.Contains("${{ vars.COVERAGE_BASELINE }}", workflow, StringComparison.Ordinal);
        Assert.Contains("${{ vars.COVERAGE_BASELINE_STORMGATE }}", workflow, StringComparison.Ordinal);
        Assert.Contains("${{ vars.COVERAGE_BASELINE_OPENTELEMETRY }}", workflow, StringComparison.Ordinal);
        Assert.Contains("gh variable set", workflow, StringComparison.Ordinal);
        Assert.Contains("continue-on-error: true", workflow, StringComparison.Ordinal);
        Assert.Contains("::warning::Coverage baseline ratchet failed", workflow, StringComparison.Ordinal);
    }
