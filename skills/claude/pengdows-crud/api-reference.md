...
with no corresponding entity (e.g. a stored procedure's output). See `docs/data-reader-mapper.md`.

```csharp
// Static entry points:
ValueTask<List<T>> DataReaderMapper.LoadAsync<T>(ITrackedReader reader, IMapperOptions? options = null, CancellationToken ct = default);
IAsyncEnumerable<T> DataReaderMapper.StreamAsync<T>(ITrackedReader reader, IMapperOptions? options = null, CancellationToken ct = default);

// Or via the singleton instance:
IDataReaderMapper mapper = DataReaderMapper.Instance;

// MapperOptions: Strict (throw on unmapped column), ColumnsOnly (skip NamePolicy),
// NamePolicy (string -> string transform, e.g. strip underscores for snake_case columns),
// EnumMode (EnumParseFailureMode).
