...
The `ARITHABORT OFF` setting specifically **prevents the SQL Server query optimizer from using indexed views**, even
when they would provide massive performance benefits.
...
performance** through proper indexed view support.

This demonstrates why pengdows.crud's SQL-first, database-aware approach is superior to Entity Framework's generic ORM
approach for performance-critical applications.
...
The benchmark will create the test database, seed data, create the indexed view, and measure the actual performance
differences across all four approaches.