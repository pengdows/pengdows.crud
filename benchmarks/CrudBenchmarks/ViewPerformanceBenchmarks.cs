...
/// <summary>
/// Thesis point #3: pengdows can do things they can't.
/// SQL Server indexed view benchmark — proves pengdows.crud treats indexed views
/// as first-class entities with automatic session settings, while Dapper and EF
/// require manual ceremony or raw SQL to hit the materialized index at all.
/// </summary>
[MemoryDiagnoser]
...
public class IndexedViewPerformanceBenchmarks : IAsyncDisposable
{
    ...
    [Benchmark]
    public async Task QueryIndexedView_Pengdows_Concurrent()
    {
    ...
    [Benchmark]
    public async Task QueryIndexedView_Dapper_Concurrent()
    {
        await BenchmarkConcurrency.RunConcurrent(OperationsPerRun, Parallelism, async () =>
        {
            await using var conn = new SqlConnection(_dapperConnStr);
    ...
    [Benchmark]
    public async Task QueryIndexedView_EntityFramework_Concurrent()
    {
        await BenchmarkConcurrency.RunConcurrent(OperationsPerRun, Parallelism, async () =>
        {
            await using var ctx = new IndexedViewEfContext(_efOptions);
            var sql = _env.BuildViewSql(param => $"@{param}");
            await ctx.CustomerOrderSummaries
                .FromSqlRaw(sql, new SqlParameter("customerId", _env.SampleCustomerId))
                .AsNoTracking()
                .FirstOrDefaultAsync();
        });
