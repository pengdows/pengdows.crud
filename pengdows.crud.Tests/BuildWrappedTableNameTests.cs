...
public class BuildWrappedTableNameTests
{
    ...
    // documented purpose of the trailing IDatabaseContext? parameter) silently emitted a COUNT
    // statement whose table name matched the *constructor's* dialect instead of the passed one.
    // Column identifiers were already correctly wrapped via sc.WrapObjectName(column), which
    // resolves against the passed context — only the table name itself was stale. Verified here
    // the same way BuildWrappedTableName_MultiDbEnvironment above verifies other SQL-building
    // methods: construct against SQLite (drops the schema prefix), pass a PostgreSQL context
    // (includes it), and confirm the schema prefix that reaches the fake connection is
    // PostgreSQL's, not SQLite's.
    // -------------------------------------------------------------------------
