...
{
    private static DatabaseContext CreateContext(fakeDbFactory factory, string? applicationName = "pengdows-test")
    {
    ...
    public async Task Standard_ReadAndWrite_UseDistinctApplicationNameSuffixes()
    {
        ...
        var readConnection = factory.CreatedConnections[^1];
        var readCs = readConnection.ConnectionStringHistory[^1];

        var writeConn = ctx.GetConnection(ExecutionType.Write);
        await writeConn.OpenAsync();
        ctx.CloseAndDisposeConnection(writeConn);

        var writeConnection = factory.CreatedConnections[^1];
        var writeCs = writeConnection.ConnectionStringHistory[^1];
