...
{
    // DatabaseContext.DetectInMemoryKind now delegates to ISqlDialect.DetectInMemoryKind rather
    // than owning its own per-product parser, so it's an instance method now — it needs _factory
    // to resolve the dialect.
    [Theory]
