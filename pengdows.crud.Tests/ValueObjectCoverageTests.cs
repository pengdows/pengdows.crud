...
public class ValueObjectCoverageTests
{
    ...
    public void PostgreSqlInterval_FromTimeSpan_PreservesPrecision()
    {
        ...
        // Days and Microseconds are non-overlapping components (matching PostgreSQL's own
        // interval model): Microseconds holds only the sub-day remainder, not
        // span.Ticks / 10 (which would double-count the 3 whole days already captured by Days).
        Assert.Equal((span.Ticks % TimeSpan.TicksPerDay) / 10, converted.Microseconds);
