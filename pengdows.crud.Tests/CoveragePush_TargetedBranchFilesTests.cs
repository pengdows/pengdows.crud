...
public sealed class CoveragePush_TargetedBranchFilesTests
{
    ...
    public void TableGateway_GetRetrieveContainer_CoversSingleAndDoubleIdBranches()
    {
        ...
        // Ported from the 2.0 branch. Production behavior differs from 2.0: a single-element id
        // list now always takes BuildWhereInternal's single-element fast path (a plain
        // "column = @p0" scalar comparison) regardless of the dialect's set-valued-parameter
        // support, rather than binding an array even for a single id — see
        // TableGateway.Retrieve.cs's BuildWhereInternal single-element fast path.
