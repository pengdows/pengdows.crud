...
public sealed class CoveragePush_TargetedBranchFilesTests
{
    ...
    public void TableGateway_GetRetrieveContainer_CoversSingleAndDoubleIdBranches()
    {
        ...
        // Even for a set-valued-parameter-capable dialect (PostgreSQL), a single-element list
        // takes BuildWhereInternal's single-element fast path, which always binds a scalar
        // equality parameter ("p0" = id) rather than an array — the ANY(@p0) array form is only
        // used once there's more than one id to match, since "id = @p0" is simpler/cheaper.
