...
public class CoverageGapTests_DialectAndTenant
{
    ...
    public void TenantContextRegistry_GetContext_UnregisteredProvider_ThrowsInvalidOperationException()
    {
        ...
        // This branch's message wording differs from the 2.0 source ("No factory registered
        // for..."): "No DbProviderFactory registered for the k..." — same condition, worded
        // differently.
        Assert.Contains("No DbProviderFactory registered for", ex.Message);
    ...
    // Regression for a TOCTOU race: the original cap check ("count >= max && !ContainsKey(tenant)")
    // and the subsequent GetOrAdd were two separate, non-atomic steps. Two threads admitting two
    // DIFFERENT new tenants could both pass the check while the count sat at cap-1, then both call
    // GetOrAdd, letting the cap be exceeded. The fix makes the check-and-add atomic under a single
    // lock (only the fast dictionary operation, not tenant construction itself, holds the lock).
    [Fact]
