...
/// </summary>
/// <remarks>
/// This exists because the same bug shape reappeared once already on a sibling branch of this
/// library (2.1.0, where this strategy is named <c>PreventDatabaseUnloadConnectionStrategy</c>
/// after a rename): its <c>GetConnectionAsync</c> was fixed to call <c>OpenAsync</c> instead of
/// the blocking <c>Open()</c>, but the fix was applied per-class, and that branch independently
/// regressed back to the sync call in the same method. A per-class regression test only protects
/// the one class it was written against; it says nothing about a future rename or a brand-new
/// <see cref="IConnectionStrategy"/> implementation.
///
