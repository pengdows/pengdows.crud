...
public sealed class PoolGovernorAsyncAcquireGapTests
{
    ...
    // Async mirror of PoolGovernorTurnstileTests.Acquire_TurnstileQueueExceedsMaxQueueDepth_
    // FailsFastInsteadOfWaitingFullTimeout, which only ever drove the SYNCHRONOUS Acquire() method
    // — AcquireAsync's own, independent turnstile-side queue-depth check had no direct coverage.
    [Fact]
    public async Task AcquireAsync_TurnstileQueueExceedsMaxQueueDepth_FailsFastInsteadOfWaitingFullTimeout()
    {
        using var turnstile = new SemaphoreSlim(1, 1);
        using var writer = new PoolGovernor(PoolLabel.Writer, "async-qd-w", 1,
            TimeSpan.FromSeconds(30), turnstile: turnstile, holdTurnstile: true);
        using var wp = await writer.AcquireAsync();

        using var reader = new PoolGovernor(PoolLabel.Reader, "async-qd-r", 1,
            TimeSpan.FromSeconds(2), trackMetrics: true, turnstile: turnstile, holdTurnstile: false,
            maxQueueDepth: 1);

        // Occupy the one allowed turnstile-queue slot with a background waiter.
        var occupierTask = Task.Run(async () =>
        {
            try { await reader.AcquireAsync(); }
            catch (PoolSaturatedException) { /* expected eventually, once the test ends */ }
        });

        await WaitUntilAsync(() => reader.GetSnapshot().TurnstileQueued >= 1, TimeSpan.FromSeconds(15));
        Assert.Equal(1, reader.GetSnapshot().TurnstileQueued);

        var sw = System.Diagnostics.Stopwatch.StartNew();
        await Assert.ThrowsAsync<PoolSaturatedException>(async () => await reader.AcquireAsync());
        sw.Stop();

        Assert.True(sw.ElapsedMilliseconds < 500,
            $"Expected an immediate PoolSaturatedException from AcquireAsync()'s turnstile queue-depth " +
            $"admission control, but it took {sw.ElapsedMilliseconds}ms.");

        await occupierTask.WaitAsync(TimeSpan.FromSeconds(10));
    }
    ...
    // TryAcquire_ExceptionAfterWinningTurnstile_ReleasesTurnstileBeforeRethrowing — see its remarks
    // for why disposing the shared semaphore is used instead of a real cancellation race.
    [Fact]
    public async Task TryAcquireAsync_ExceptionAfterWinningTurnstile_ReleasesTurnstileBeforeRethrowing()
    {
        using var turnstile = new SemaphoreSlim(1, 1);
        var sharedSemaphore = new SemaphoreSlim(1, 1);
        using var writer = new PoolGovernor(PoolLabel.Writer, "async-tryacquire-cleanup-key", 1,
            TimeSpan.FromSeconds(10), sharedSemaphore: sharedSemaphore, turnstile: turnstile, holdTurnstile: true);

        sharedSemaphore.Dispose();

        await Assert.ThrowsAsync<ObjectDisposedException>(async () => await writer.TryAcquireAsync());

        Assert.True(turnstile.Wait(0), "Turnstile should have been released by TryAcquireAsync's cleanup catch block.");
        turnstile.Release();
    }
