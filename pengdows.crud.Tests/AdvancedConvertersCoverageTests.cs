...
public class AdvancedConvertersCoverageTests
{
    ...
    public void MacAddressConverter_FormatsForPostgres_AndReadsPhysicalAddress()
    {
        ...
        // This branch's MacAddressConverter.ConvertToProvider returns the underlying
        // PhysicalAddress for every provider (Npgsql's macaddr handler accepts PhysicalAddress
        // directly; a string with NpgsqlDbType.MacAddr is rejected during preparation) rather
        // than a pre-formatted colon-separated string.
        var providerValue = converter.ToProviderValue(mac, SupportedDatabase.PostgreSql);
        Assert.IsType<PhysicalAddress>(providerValue);
        Assert.Equal(mac.Address, providerValue);
    ...
    public void IntervalYearMonthConverter_FormatsIso_ForOracle()
    {
        ...
        // This branch's IntervalYearMonthConverter formats Oracle's own native
        // INTERVAL YEAR TO MONTH literal ("+YYYY-MM") for the Oracle provider rather than ISO
        // 8601 — only PostgreSql/CockroachDb get the ISO 8601 "PxYyM" form. Round-tripping via
        // FromProviderValue still accepts ISO 8601 input regardless of provider (Parse doesn't
        // branch on provider), which the assertions below still exercise.
    ...
    public void IntervalDaySecondConverter_FormatsIso_ForOracle()
    {
        ...
        // This branch's IntervalDaySecondConverter formats Oracle's own native
        // INTERVAL DAY TO SECOND literal ("+DDDDDDDDD HH:MM:SS.ffffff") for the Oracle provider
        // rather than ISO 8601 — only PostgreSql/CockroachDb get the ISO 8601 "PxDTyHzMwS" form.
        var providerValue = converter.ToProviderValue(interval, SupportedDatabase.Oracle);
    ...
    public void PostgreSqlIntervalConverter_FormatsIso_ForPostgres()
    {
        ...
        // This branch's PostgreSqlIntervalConverter returns a plain TimeSpan (via
        // PostgreSqlInterval.ToTimeSpan()) for PostgreSql/CockroachDb/YugabyteDb rather than an
        // ISO 8601 string — Npgsql binds INTERVAL values from TimeSpan directly, and a string
