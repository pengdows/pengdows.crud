...
public class CoverageGapTests_TypesAndConverters
{
    ...
    // The tests below were originally written against a design where
    // PostgreSqlIntervalConverter.ConvertToProvider formatted PostgreSqlInterval as an ISO 8601
    // duration string for PostgreSql/CockroachDb. This branch's converter instead returns a
    // plain TimeSpan (via PostgreSqlInterval.ToTimeSpan()) for those providers — Npgsql binds
    // PostgreSQL INTERVAL values from TimeSpan directly, and a string combined with
    // NpgsqlDbType.Interval is rejected during preparation. ToTimeSpan() is also lossy: it drops
    // Months entirely (see PostgreSqlInterval.ToTimeSpan doc comment), which several assertions
    // below now confirm directly. The private FormatIso8601 helper still exists in the converter
    // but is dead code — nothing calls it anymore.
    ...
    public void PostgreSqlIntervalConverter_ConvertToProvider_CockroachDb_ReturnsIso8601()
    {
        ...
        Assert.IsType<TimeSpan>(result);
        // Months-only interval: ToTimeSpan() drops Months, leaving TimeSpan.Zero.
    ...
    public void PostgreSqlIntervalConverter_FormatIso8601_MonthsAndDaysAndTime()
    {
        ...
        // Months (2) are dropped by ToTimeSpan(); only Days + the sub-day time component survive.
    ...
    {
        // This branch's IntervalYearMonthCoercion added an int/long fast path (ODP.NET may
        // expose INTERVAL YEAR TO MONTH as the provider's numeric total-months representation
        // after a cached statement is reused), so `int` is no longer an "unknown" raw type here.
        // Use a genuinely unhandled type (double) to preserve this test's original intent.
        var coercion = new IntervalYearMonthCoercion();
        var src = new DbValue(42d);
