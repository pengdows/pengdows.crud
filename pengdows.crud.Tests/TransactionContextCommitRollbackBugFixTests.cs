...
/// <summary>
/// Regression tests (backported from 3.0):
///
...
/// - CompleteTransactionAsync wrapped ANY exception from that call — including
///   OperationCanceledException — into TransactionException, violating the documented contract
///   that OperationCanceledException is never wrapped.
/// </summary>
public class TransactionContextCommitRollbackBugFixTests
{
    ...
    [Fact]
    public async Task DisposeAsync_WhenNotCompleted_RollsBackViaRollbackAsync()
    {
        var (context, txn, fakeTransaction) = CreateOpenTransaction();
        using var _ = context;
