...
public class DataSourceInformationAsyncTests
{
    ...
    {
        var method =
            typeof(DataSourceInformation).GetMethod("IsSqliteAsync", BindingFlags.NonPublic | BindingFlags.Static)!;
    ...
    {
        var method =
            typeof(DataSourceInformation).GetMethod("GetVersionAsync", BindingFlags.NonPublic | BindingFlags.Static)!;
        var task = (Task<string>)method.Invoke(null,
            new object?[] { conn, sql, NullLogger<DataSourceInformation>.Instance })!;
        return await task.ConfigureAwait(false);
    }
