...
public class CoveragePush_DialectMissingPathsTests
{
    ...
    // MySql_DetermineStandardCompliance_* (3 tests) dropped when porting from the 2.0 branch:
    // MySqlDialect.DetermineStandardCompliance / SqlStandardLevel do not exist anywhere in this
    // branch's production code — the standard-compliance-level concept was removed entirely (see
    // SqlDialect.cs / IDatabaseProductInfo.cs; also noted against SqlDialectAdditionalCoverageTests.cs
    // and the skipped SqlStandardComplianceTests.cs/SqlStandardLevelTests.cs).
