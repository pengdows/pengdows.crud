...
public class Db2DialectTests
{
    ...
    [Fact]
    public void BuildCreateWithReturning_Db2_WrapsEntireInsertInFinalTable()
    {
        // Db2 has no dialect-owned "wrap the whole statement" hook (unlike newer branches) —
        // TableGateway.Core.cs (BuildCreateWithReturning) special-cases SupportedDatabase.Db2
        // directly, matching its existing SqlServer/Oracle special cases. This verifies that
        // wiring end-to-end rather than at the dialect layer.
        var factory = new fakeDbFactory(SupportedDatabase.Db2);
        var context = new DatabaseContext("Data Source=test;EmulatedProduct=Db2", factory);
        var helper = new TableGateway<Db2ReturningEntity, int>(context);

        var entity = new Db2ReturningEntity { Name = "test" };
        var container = helper.BuildCreateWithReturning(entity, true, context);
        var sql = container.Query.ToString();

        Assert.StartsWith("SELECT \"Id\" FROM FINAL TABLE (", sql);
        Assert.EndsWith(")", sql);
        Assert.Contains("INSERT INTO", sql);
        Assert.Contains("VALUES", sql);
    }

    [Table("db2_returning_entity")]
    private sealed class Db2ReturningEntity
    {
        [Id(false)]
