...
public class MySqlDialectUpsertTests
{
    [Fact]
    public async Task UpsertIncomingColumn_UsesAlias_OnModernMySql()
    {
        ...
        var dialect = new MySqlDialect(factory, NullLogger.Instance);
    ...
    [Fact]
    public async Task UpsertIncomingColumn_UsesValues_OnLegacyMySql()
    {
        var factory = new fakeDbFactory(SupportedDatabase.MySql);
        using var connection = new fakeDbConnection();
        connection.EmulatedProduct = SupportedDatabase.MySql;
