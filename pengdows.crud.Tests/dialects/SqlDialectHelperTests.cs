...
public class SqlDialectHelperTests
{
    ...
    public void GetNaturalKeyLookupQuery_ThrowsForEmptyKeys()
    {
        ...
        Assert.Throws<InvalidOperationException>(() =>
            dialect.GetNaturalKeyLookupQuery("table", "id", Array.Empty<string>(), Array.Empty<string>()));
    }
    ...
    public void GetNaturalKeyLookupQuery_ThrowsForMismatchedCounts()
    {
        ...
        Assert.Throws<ArgumentException>(() =>
            dialect.GetNaturalKeyLookupQuery("table", "id", new[] { "name" }, Array.Empty<string>()));
    }
    ...
    private static (string Settings, bool UsedFallback, IReadOnlyDictionary<string, string> Snapshot)
        CallSessionSettingsResult(SqlDialect dialect, string methodName, IDbConnection connection)
    {
    ...
    {
        return new TestDialect(new fakeDbFactory(SupportedDatabase.Unknown),
            NullLoggerFactory.Instance.CreateLogger(nameof(TestDialect)));
    }
    ...
    {
        return new NaturalKeyDialect(new fakeDbFactory(SupportedDatabase.Unknown),
            NullLoggerFactory.Instance.CreateLogger(nameof(NaturalKeyDialect)), product, supportsIdentityColumns);
    }

    private sealed class TestDialect : SqlDialect
    {
        public TestDialect(DbProviderFactory factory, Microsoft.Extensions.Logging.ILogger logger) : base(factory,
            logger)
        {
        }
        ...
        public bool TestIsVersionAtLeast(int major, int minor = 0, int build = 0)
        {
            return IsVersionAtLeast(major, minor, build);
        }
    }
    ...
    private sealed class NaturalKeyDialect : SqlDialect
    {
        ...
        public NaturalKeyDialect(DbProviderFactory factory, Microsoft.Extensions.Logging.ILogger logger,
            SupportedDatabase product, bool supportsIdentityColumns)
            : base(factory, logger)
        {
            _product = product;
    ...
    private sealed class SessionSettingsConnection : IDbConnection
    {
        ...
        {
            return new SessionSettingsCommand(_rows);
        }
    ...
    private sealed class ThrowingSessionConnection : IDbConnection
    {
        ...
        public IDbCommand CreateCommand()
        {
            return new ThrowingSessionCommand();
        }
    ...
    private sealed class SessionSettingsReader : IDataReader
    {
        ...
        public int FieldCount =>
            _index >= 0 ? _rows[_index].ColumnNames.Length : _rows.FirstOrDefault()?.ColumnNames.Length ?? 0;
        ...
        {
            return _rows[_index].ColumnNames.ElementAtOrDefault(i) ?? string.Empty;
        }

        public int GetOrdinal(string name)
        {
            return Array.IndexOf(_rows.FirstOrDefault()?.ColumnNames ?? Array.Empty<string>(), name);
        }
    ...
    private sealed class SessionSettingRow
    {
        ...
        public SessionSettingRow Clone()
        {
            return new SessionSettingRow(ColumnNames.ToArray(), Values.ToArray());
        }
    }
    ...
    private sealed class StubColumnInfo : IColumnInfo
    {
        ...
        public PropertyInfo PropertyInfo { get; init; } = typeof(object).GetProperty(nameof(ToString))!;
