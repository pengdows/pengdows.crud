...
public class InfrastructureTimeoutExceptionIdentityTests
{
    ...
    public async Task ModeContentionException_PropagatesUnwrapped_FromOrdinaryOpDuringActiveTransaction_NotTranslatedToCommandTimeoutException()
    {
        ...
        // CommandTimeoutException. ExecuteNonQueryAsync specifically: unlike 3.0, this branch's
        // SingleConnection transaction gate (GetSingleConnectionTransactionGateForOrdinaryOp) is
        // only acquired on the ExecuteNonQueryAsync path, not by every execution method - verified
        // directly against SqlContainer.cs rather than assumed.
        using var sc = context.CreateSqlContainer("DELETE FROM t WHERE 1 = 0");

        var ex = await Assert.ThrowsAsync<ModeContentionException>(
            () => sc.ExecuteNonQueryAsync(ExecutionType.Write, CommandType.Text).AsTask());
