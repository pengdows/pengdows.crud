...
{
    /// <summary>
    /// The 2.0-era assumption behind this test's name ("SafeNonBlockingReads requires RCSI — a
    /// SQL Server-only feature — so PostgreSQL must throw") no longer holds in this branch:
    /// PostgreSqlDialect.GetIsolationProfileMapping now deliberately maps SafeNonBlockingReads to
    /// RepeatableRead (PostgreSQL's REPEATABLE READ is fully non-blocking and additionally
    /// prevents phantom reads for the transaction's lifetime — see the extensive comment on that
    /// override), so the profile is supported, not rejected.
    /// </summary>
    [Fact]
    public async Task BeginTransactionAsync_SafeNonBlockingReadsOnPostgreSql_Throws()
    {
        ...
        Assert.Equal(IsolationLevel.RepeatableRead, txn.IsolationLevel);
    }
