...
{
    /// <summary>
    /// Ported from the 2.0 branch, where this documented that SafeNonBlockingReads requires
    /// RCSI (a SQL Server-only feature) and therefore had no PostgreSQL equivalent. Production
    /// behavior differs from 2.0: PostgreSqlDialect now has an explicit
    /// GetIsolationProfileMapping entry mapping SafeNonBlockingReads to RepeatableRead — PostgreSQL's
    /// REPEATABLE READ is fully non-blocking and snapshot-based, making it the correct match for
    /// this profile without needing RCSI (see PostgreSqlDialect.cs's GetIsolationProfileMapping
    /// remarks). This async path now succeeds rather than throwing.
    /// </summary>
    [Fact]
