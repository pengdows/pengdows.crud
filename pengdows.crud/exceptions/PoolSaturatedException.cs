...
//   * Snapshot: PoolStatisticsSnapshot with InUse, MaxSlots, Queued counts
...
public sealed class PoolSaturatedException : TimeoutException
{
    public PoolSaturatedException(PoolLabel label, string poolKeyHash, PoolStatisticsSnapshot snapshot,
        TimeSpan timeout)
        : base(
            $"{label} pool saturated: {snapshot.InUse}/{snapshot.MaxSlots} in use, {snapshot.Queued} queued, timed out after {timeout} (key {poolKeyHash}).")
    {
        PoolLabel = label;
