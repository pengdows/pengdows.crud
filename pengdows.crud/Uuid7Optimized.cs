...
//   * Chronologically sortable for stores that compare the UUID byte pattern directly
//     (e.g. PostgreSQL, MySQL, SQLite) — NOT automatically true for SQL Server's
//     `uniqueidentifier`, whose comparison semantics don't sort by RFC byte order.
//   * Contains timestamp (useful for debugging/auditing)
//   * No index fragmentation issues like random UUIDs, again for byte-order-comparing stores
...
/// <param name="MaxNegativeSkewMs">
/// Threshold (ms) for treating a backward clock jump as "small" (pin to the thread's own last
/// timestamp) vs. "large" (adopt the cross-thread global epoch instead, if it's further ahead).
/// Neither branch resyncs to the actual observed clock value on a backward jump — both use a
/// held-back logical timestamp; the cross-thread epoch adoption is the only behavior this
/// threshold selects.
/// </param>
...
///
/// Monotonicity scope: PER THREAD only, not process-wide. Each thread has independent,
/// randomly-reseeded state (see V7ThreadState), so two UUIDs generated by different threads in
/// the same millisecond are NOT guaranteed to sort in call order — only same-thread,
/// same-millisecond generation is strictly increasing. The shared global epoch coordinates the
/// millisecond timestamp floor across threads (a thread will never pick an earlier millisecond
/// than one already observed process-wide), but it does not coordinate the intra-millisecond
/// sequence field. Not guaranteed across machines.
...
/// Clock rollback: Bounded drift with logical clock fallback. Blocking on counter exhaustion
/// during a large backward clock jump is intentionally unbounded in wall-clock duration (only
/// the spin/sleep cadence is bounded) — see BoundedWaitNextMs — trading worst-case latency for
/// strict per-thread ordering. Set FailFastOnBurst to throw instead of blocking if that
/// trade-off is wrong for your workload.
/// </summary>
public static partial class Uuid7Optimized
{
    ...
    /// <summary>
    /// Generate a UUIDv7 directly to a byte span.
    ...
    // "Bounded" describes the spin/sleep cadence (MaxSpinCount, SleepMs), not the total wait
    // duration: the loop below blocks until the wall clock exceeds currentMs, so a large
    // backward clock jump can hold a thread here for as long as that jump lasts. This is
    // intentional — it preserves strict per-thread ordering rather than emitting an
    // out-of-order UUID — but it means counter exhaustion during a severe clock rollback can
    // produce a correspondingly long block. Configure FailFastOnBurst if blocking indefinitely
    // is worse for your workload than throwing.
