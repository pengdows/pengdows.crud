...
public class SqlContainer : SafeAsyncDisposableBase, ISqlContainer, ISqlDialectProvider, IReaderLifetimeListener
{
    ...
    /// </summary>
    /// <remarks>
    /// This deliberately checks type identity, not structural shape: treating any exception that
    /// exposes a property named "Number"/"SqlState"/"NativeError" (or an "Errors" collection) as
    /// provider-originated would misclassify an unrelated application exception that merely
    /// happens to expose a similarly-named property — discarding its real type and mistaking an
    /// application bug for a database failure to a caller doing <c>catch (DatabaseException)</c>.
    /// Almost every real ADO.NET provider's exception type derives from <see cref="DbException"/>;
    /// the only known exception is checked explicitly via
    /// <see cref="KnownNonDbExceptionProviderExceptionTypeNames"/>.
    /// </remarks>
    ...
    /// <summary>
    /// Async counterpart of <see cref="GetConnection"/> -- genuinely awaits pool-slot
    /// acquisition instead of blocking the calling thread. Used by every async execution path
    /// below (ExecuteReaderAsyncInternal, ExecuteNonQueryAsync, etc.) in place of the
    /// synchronous <see cref="GetConnection"/>. See AsyncConnectionAcquisitionTests for the
    /// regression this exists to prevent (sync-over-async CLR ThreadPool starvation).
    /// </summary>
