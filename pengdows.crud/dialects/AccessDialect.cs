...
//
// Ported from pengdows.crud 3.0 and adapted to this 2.0.6 patch line, matching how
// Informix/SapHana/InterBase/Spanner were each already added here. Two pieces of 3.0's
// AccessDialect don't carry over because the corresponding ISqlDialect/SqlDialect hooks don't
// exist on this branch yet (2.0.6 predates 3.0's per-dialect isolation/natural-key-select hook
...
//     InterBase cases there).
// Everything else below is unchanged from 3.0 — CoerceConnectionMode, DescribeStandardModeRisk,
// TryClassifyProviderException's Access case (added to SqlDialect.cs's existing centralized
// switch, matching the Informix/SapHana/Spanner precedent already there), IsUniqueViolation/
// IsForeignKeyViolation/IsNotNullViolation/IsCheckConstraintViolation, and every other member all
// port directly since the hooks they depend on (IsClientServerDatabase,
// IsEmbeddedSingleWriterEngine, DetectInMemoryKind, CoerceConnectionMode,
// ReadOnlyPoolDiscriminatorSettingName/Value, CoerceEmbeddedSingleWriterMode's allowStandard
// parameter, DescribeStandardModeRisk) already exist as virtual members on this branch's
// SqlDialect base class.
//
// AI SUMMARY:
...
internal sealed class AccessDialect : SqlDialect
{
    ...
    // CONFIRMED live (this session, via a real .accdb): Jet/ACE has no OFFSET/FETCH or
    // LIMIT/OFFSET paging syntax of any kind — its only paging idiom is SELECT TOP n (see the
    ...
    // CONFIRMED live (this session, via a real .accdb): Jet/ACE rejects the ANSI multi-row
    // "INSERT INTO t (...) VALUES (...), (...)" syntax outright ("Syntax error (missing
    // operator)") — same limitation as Informix/SAP HANA/InterBase, all of which also override
    // this to false. Falls back to one INSERT per entity (BuildBatchCreate's SupportsBatchInsert
    // check).
    ...
    // advisory AnalyzeException API and, via AccessExceptionTranslator's own message matching,
    // the actual thrown exception type (CommandTimeoutException, IsTransient = true) — so a
    // caller retrying on IsTransient does the right thing instead of this falling through to a
    // generic DatabaseOperationException. The category-classification side of this (feeding
    // ISqlDialect.AnalyzeException/ClassifyException) is an inline SupportedDatabase.Access case
