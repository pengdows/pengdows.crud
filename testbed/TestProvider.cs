            ...
            case SupportedDatabase.SingleStore:
                {
                    ...
                    // SingleStore does NOT accept MySQL's bare "CREATE PROCEDURE proc() SELECT 42"
                    // form (real syntax error: "unexpected end of function definition" / "syntax
                    // error at or near SELECT" — confirmed against a live singlestoredb-dev
                    // container). SingleStore's own procedural SQL dialect requires an explicit
                    // AS BEGIN...END block, and returning a result set from a procedure requires
                    // the SingleStore-specific ECHO statement rather than a bare SELECT. CALL syntax
                    // and quoted-identifier handling are otherwise identical to MySQL/MariaDB, and
                    // this is a single ADO.NET command (embedded semicolons included) exactly like
                    // the MySQL/MariaDB case above — no DELIMITER handling needed, confirmed via a
                    // real MySqlConnector round trip against SingleStore.
                    sc.Query.Append($"CREATE PROCEDURE {singleStoreProcName}() AS BEGIN ECHO SELECT 42; END");
                    await sc.ExecuteNonQueryAsync();
