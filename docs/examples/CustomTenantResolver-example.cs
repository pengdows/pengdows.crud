...
//   await during the rotation should use AcquireLease instead of a bare
//   GetContext reference (see multitenancy.md's "Protecting against
//   concurrent rotation" section) so it isn't disposed out from under it
//   mid-use.
...
public sealed class SqlControlPlaneTenantResolver : ITenantConnectionResolver
{
    ...
    /// it acquired that context via <c>AcquireLease</c> rather than a bare <c>GetContext</c>
    /// reference — see multitenancy.md's "Protecting against concurrent rotation" section.
    /// </summary>
...
// //    match the provider's ADO.NET invariant name (see dynamic-provider-loading.md's
// //    "DI registration key vs. ProviderName" section for the same contract).
...
// //    these for you; a fully custom setup registers them directly instead. (TenantContextRegistry
// //    also needs ILoggerFactory, which the host builder's default logging registration provides.)
...
// //    Optional tenant-cardinality cap: register the registry via a factory instead, e.g.
// //    sp => new TenantContextRegistry(sp, sp.GetRequiredService<ITenantConnectionResolver>(),
// //        sp.GetRequiredService<IDatabaseContextFactory>(), sp.GetRequiredService<ILoggerFactory>(),
// //        maxTenantCount: 500)
//
