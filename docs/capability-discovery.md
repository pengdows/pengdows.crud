...
`ISqlDialect` (`pengdows.crud.abstractions/dialects/ISqlDialect.cs`, ~900 lines) is CLAUDE.md's
...
2. **Internal SQL-generation machinery** — methods `TableGateway`/`PrimaryKeyTableGateway` call to
   ...
   `GetCompoundInsertIdSuffix`, `GetLastInsertedIdFromCommand`,
...
inspection) — not a documentation oversight you're missing. Until/unless it's split into two interfaces, the practical rule is: if
...
`RequiresMergeStatementTerminator`, `MergeUpdateRequiresTargetAlias`,
...
**Stored procedures:** `ProcWrappingStyle` (see `docs/stored-procedures.md` for the
...
`MaxPoolSizeSettingName` — see `docs/connection-pooling.md` for how these feed pool
configuration.
...
kind, transient/retryable signals) — see `docs/exception-analysis.md` for the dedicated
...
which has no such alias — a real, shipped bug. The cached fragment is still pre-built in
`MERGE` form whenever `SupportsMerge` is true (`TableGateway.Sql.cs`'s `upsertUpdateFragment`
pre-build); the batch path fixes it by checking *both* flags and rewriting the `s.` source-alias
references to `EXCLUDED.` when a dialect supports `MERGE` and `ON CONFLICT` at the same time
(see `TableGateway.Batch.cs`'s batch-upsert builder and its inline comment). The lesson generalizes: check the specific flags relevant to the *specific
...
- **Additive changes are low-risk.** Several members are C# default-interface-method members
  (`IsPrepareExhausted`, `RequiresMergeStatementTerminator`, `SupportsOnConflictWhere`,
  `GetLastInsertedIdFromCommand`, `ClassifyException`, `AnalyzeException`, `WrapSimpleName`,
  `ReplaceNeutralTokens`, `RenderMergeOnClause`, `IsForeignKeyViolation`, `IsNotNullViolation`,
  `IsCheckConstraintViolation`) specifically so a new capability flag can be added
