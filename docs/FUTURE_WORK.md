...
| VAR-003 | **Decision (maintainer, 2026-09-26): Db2 for z/OS and Db2 for i get their own `SupportedDatabase` values and dialects** — separate IBM products with real differences beyond the common SQL subset (e.g. z/OS catalogs/versioning/function levels; IBM i system naming, journaling-dependent commitment control, QSYS2 catalogs, release+PTF versioning — to be confirmed by testing). `SupportedDatabase.Db2` keeps meaning LUW, matching IBM's own naming ("IBM Db2" for LUW vs "Db2 for z/OS" / "Db2 for i"). Add the values (e.g. `Db2ForZOS`, `Db2ForI`) only when each can be built and tested against a real server: first capture what IBM's .NET driver reports for each (on LUW: `ServerType`/`DataSourceProductName` = `DB2/<platform>`; PRDID not exposed; `GET DIAGNOSTICS … DB2_PRODUCT_ID` is z/OS-only syntax) and replay it in fakeDb. Until then only Db2 LUW is supported (maintainer decision, 2026-09-26 — transactions differ between the products); a Db2 server not recognized as LUW logs a warning and Best selects PreventDatabaseUnload only on a positively detected LUW server. When the values arrive, those users' `context.Product` changes from `Db2` (acceptable: they are documented as unsupported until then) | Future | Open (needs a z/OS / IBM i connection) |
