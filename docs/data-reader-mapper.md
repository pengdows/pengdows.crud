...
var summaries = await DataReaderMapper.LoadObjectsFromDataReaderAsync<OrderSummary>(reader);
```
...
Both a static, allocation-free entry point (`DataReaderMapper.LoadObjectsFromDataReaderAsync<T>(...)`,
`DataReaderMapper.LoadAsync<T>(...)`, `DataReaderMapper.StreamAsync<T>(...)`) and a DI-friendly singleton instance
...
IAsyncEnumerable<T> StreamAsync<T>(ITrackedReader reader, CancellationToken ct = default);
...
| Option | Effect |
|---|---|
| `Strict` | Throws `DataMappingException` when a matched column's value can't be converted/assigned to its property. Default `false`: the failure is logged as a warning via `TypeCoercionHelper.Logger` and the property keeps its default. Reader columns with no matching property are ignored either way. |
| `ColumnsOnly` | When `true`, only properties carrying a `[Column]` attribute are mapped, matched (case-insensitively) against the `[Column]` name rather than the property name, and `NamePolicy` is not applied. |
