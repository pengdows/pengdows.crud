...
own multi-thousand-test unit suite runs through it. It is not a toy: it emulates every non-`Unknown`
`SupportedDatabase` product's schema/version-detection responses, injects failures at any layer of the connection lifecycle,
...
identifier quoting, all of it. Every non-`Unknown` `SupportedDatabase` member has an emulation:
`PostgreSql`, `SqlServer`, `Oracle`, `Firebird`, `CockroachDb`, `MariaDb`, `MySql`, `Sqlite`,
`DuckDB`, `YugabyteDb`, `TiDb`, `Snowflake`, `AuroraMySql`, `AuroraPostgreSql`, `Db2`, `FlatFile`,
`SingleStore`, `SybaseASE`, `Spanner`, `Informix`, `SapHana`, `InterBase`, `Access`. `EmulatedProduct=X` in the connection string
...
Each emulated product has an embedded schema resource (`pengdows.crud.fakeDb/xml/<Product>.schema.xml`, served by `GetSchema()`/`GetSchema(string)`) that
