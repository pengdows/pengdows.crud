...
- **`ExecutionType`** — pass `ExecutionType.Write` for any statement that mutates data or must land
  ...
  see the `ExecuteScalarRequiredAsync` remarks in code: using the wrong intent for a
  generated-ID read routes it to the wrong connection/pool). Pass `ExecutionType.Read` for
  ordinary queries. Omitting it entirely uses whichever default the specific method documents —
  check the method's own doc comment rather than assuming, since `ExecuteScalarOrNullAsync()`'s
  parameterless overload hardcodes `ExecutionType.Read`.
...
- **`BuildWhereByPrimaryKey(...)`** (on `ITableGateway`/`IPrimaryKeyTableGateway`) adds a parameterized natural-key
...
- **`ClearCaches()`** clears a gateway's compiled reader plans, column lists, query templates, and
  cached WHERE parameter names. This is gateway-lifetime state, intentionally not tied to any one
  operation context (see `docs/entity-mapping.md`) — call this only after a schema or mapping
...
- **`GetPoolStatisticsSnapshot(PoolLabel)`** reports slot usage, queue depth, turnstile waits,
  ...
  [`connection-pooling.md`](./connection-pooling.md) and
