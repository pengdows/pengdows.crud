...
target (property, except `[Table]` on the class and `[EnumLiteral]` on an enum field); only
`[Table]` and `[EnumLiteral]` explicitly set `Inherited = false`. Validation happens once, at `TypeMapRegistry.BuildTableInfo` (first access
...
`System.Data.DbType` used for parameter creation and drives enum storage format (any string
`DbType` — `String`, `AnsiString`, `StringFixedLength`, `AnsiStringFixedLength` — → enum stored as
its name; a numeric `DbType` → stored as its underlying numeric value; anything
...
- On `TableGateway<T,TId>`, a `[Version]` column also enables `loadOriginal`-by-default
  ...
  genuinely changed columns are included, alongside the version predicate.
  `PrimaryKeyTableGateway<T>` ignores `loadOriginal`.
