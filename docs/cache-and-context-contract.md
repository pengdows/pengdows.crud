...
|---|---|---|---|---|
| `DataReaderMapper._planCache` | Static, process-wide | `(Type, RecordsetShape, ColumnsOnly, EnumMode)` | 128 (`MaxPlanCacheSize`) | Confirmed dead code externally (no public factory/DI path) — see `docs/planning/future-work.md`'s CORE-013 history. Not tenant-cardinality-related; keyed by entity type + result shape. |
| `DataReaderMapper._setterCache` | Static, process-wide | `SetterCacheKey` (property-level) | 512 (`MaxSetterCacheSize`) | Same reachability caveat as above. |
| `DataReaderMapper._propertyLookupCache` | Static, process-wide | `PropertyLookupCacheKey` | 64 (`MaxPropertyLookupCacheSize`) | Same reachability caveat as above. |
