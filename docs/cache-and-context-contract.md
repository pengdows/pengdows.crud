...
| `ConnectionStringNormalizationCache` (`pengdows.crud/internal/ConnectionStringNormalizationCache.cs`) | Static, process-wide | The **raw connection string** (ordinal comparison) | **Unbounded** `ConcurrentDictionary` — no eviction | Populated only when a context compares its primary and read-only connection strings for equivalence (`AreConnectionStringsEquivalentIgnoringCredentials`). Values are credential-scrubbed before storage (password/user/secret/token/access-style keys are dropped from the normalized map), but the **key is the original string, credentials included**, and entries are never evicted. A long-lived process that constructs contexts for many distinct per-tenant connection strings accumulates one entry per distinct string for the process lifetime. |

**Secret handling summary:** `ConnectionStringNormalizationCache` is the only cache in this table
with connection-string (and therefore potential-credential) exposure: its values are scrubbed, but
its keys are the raw connection strings, held in process memory without eviction. No other cache in
