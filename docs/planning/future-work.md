...
| TEST-009 | P1 | **Done for `BaseTableGateway` (2026-08-29)** | Reader-plan collision safety | Force equal hash values for distinct schemas and verify both gateway and general mapper select structurally correct plans. Include adversarial aliases, reordered columns, and different field types. Hash-collision case already covered by CORE-013's `MapReaderToObject_TwoDistinctShapesWithCollidingHash_CacheTwoSeparatePlans`. **New:** confirmed `RecordsetShape.Equals` compares `_types[i]`/`_names[i]` by INDEX (not as an unordered set), so it was already correctly order-sensitive and type-sensitive — no bug found, but nothing previously proved it at the cache level. Strengthened `MapReaderToObject_DifferentFieldTypes_BuildsSeparatePlans` (its name promised a cache-level guarantee its body never actually checked — it only asserted output values, which could pass even with one generically-coercing shared plan) with a `_readerPlans.Count == 2` assertion. Added `MapReaderToObject_ReorderedColumns_BuildsSeparatePlans` — same column names/types, same field count, opposite declaration order — proving two separate plan-cache entries rather than one plan's ordinal-position assumptions silently misreading a differently-ordered reader. "Adversarial aliases" is covered by the same by-name, `OrdinalIgnoreCase` comparison already exercised across all these cases; no further scenario was found where an alias could defeat it. **Deliberately not extended to `DataReaderMapper`** ("general mapper"), consistent with CORE-013's own scoping: it is confirmed dead code externally (`internal sealed`, no public factory/DI path, not what `TableGateway`/`PrimaryKeyTableGateway` use for hydration) — writing new tests against unreachable code was judged not worth the effort here either. Full suite green (7398 tests, both net8.0/net10.0; one `ReusableAsyncLockerTests.LockAsync_Contended_WaitsUntilReleased` timing flake during the batched run was confirmed unrelated — passed cleanly on an immediate full-suite rerun). |
| TEST-010 | P1 | **Partially Done (2026-08-29)** | Generated-ID connection affinity | Make the provider return different physical sessions on consecutive leases. Prove every session-scoped generated-ID strategy remains on the insert connection or fails without mutating the entity ID. **Full "different physical sessions on consecutive leases" simulation is genuinely not buildable in fakeDb without significant new per-connection session-state infrastructure** (fakeDb connections have no session state to diverge in the first place) — consistent with CORE-016's own conclusion that this hazard needs a real-provider testbed to fully verify. **What fakeDb *can* prove, and now does:** connection-instance *labeling* (read vs. write), which is enough to catch the concrete defect this investigation actually found — `GeneratedIdConnectionAffinityTests.CreateAsync_CompoundStatementFallback_NeverUsesAReadConnectionForTheGeneratedIdQuery` asserts no connection carrying MySQL's read-only session setting (`transaction_read_only = 1`) is ever used during a `CreateAsync` that goes through `PopulateGeneratedIdAsync`'s fallback; confirmed genuinely red first (the fallback was unconditionally acquiring a read-labeled connection via a hardcoded-`ExecutionType.Read` overload — see CORE-016's updated row for the fix). Full suite green (7399 tests, both net8.0/net10.0). **Still open, matching CORE-016:** proving affinity to the *exact same physical connection instance* as the INSERT (not just "the correct pool") requires either a real provider or new fakeDb per-connection session-value infrastructure — deferred, not attempted here. |
