...
`TransactionContext` drives every explicit `BeginTransactionAsync` call. The factory invokes `context.GetConnection` with the resolved `ExecutionType` so the configured connection strategy (Standard/SingleWriter/SingleConnection) can pick the right physical connection, and the connection is opened before the transaction starts. CockroachDB always moves to `IsolationLevel.Serializable`, and read-only contexts are prohibited from opening write transactions (`NotSupportedException` if the caller requests `ExecutionType.Write` while the context is read-only). A dedicated `SemaphoreSlim`, wrapped by `ReusableAsyncLocker`, guards the logical user lock so the caller can still buffer async work inside the transaction without racing commit/rollback.

**DuckDB is a capability-declared exception, not just "prefers the default."** `DuckDbDialect.RejectsExplicitIsolationLevelOnBeginTransaction => true` declares that DuckDB's own ADO.NET provider actually rejects an explicit `IsolationLevel` argument passed to `BeginTransaction` — the provider only supports `Serializable` in the first place (`GetSupportedIsolationLevels`/`GetIsolationProfileMapping` both map every level/profile to `Serializable`). `TransactionContext` honors this flag by omitting the explicit `IsolationLevel` argument when calling the provider's own `BeginTransaction`, while still preserving and reporting the logically-resolved isolation level for policy/observability purposes — the same pattern used elsewhere for provider capability differences: the constraint lives on the dialect, not as an `if (product == DuckDB)` special case in `TransactionContext` itself.
...
| `SavepointAsync(string name, CancellationToken)` | Creates a named savepoint. Throws `NotSupportedException` if the dialect doesn't support savepoints at all (`ISqlDialect.SupportsSavepoints`). Returns `ValueTask`. |
| `RollbackToSavepointAsync(string name, CancellationToken)` | Rolls back to a named savepoint without ending the transaction. Throws `NotSupportedException` on an unsupported dialect. Returns `ValueTask`. |
| `ReleaseSavepointAsync(string name, CancellationToken)` | Explicitly discards a savepoint before the transaction ends. Throws `NotSupportedException` when the dialect's `ISqlDialect.SavepointCapabilities` doesn't include `Release` — SQL Server, Sybase, and Oracle support creating and rolling back savepoints but have no explicit release statement at all. Returns `ValueTask`. |
...
`CommitAsync`/`RollbackAsync` route through `CompleteTransactionWithWaitAsync`, which serializes completion behind a semaphore so commits/rollbacks never overlap. `SavepointAsync`/`RollbackToSavepointAsync`/`ReleaseSavepointAsync` all fail fast with `NotSupportedException` rather than silently no-op when the dialect doesn't support the requested operation — a savepoint operation that silently did nothing used to let a caller believe partial work was protected/undone when it wasn't. When supported, the dialect's SQL is executed on the same transaction so you can roll back (or release) a subset of work without leaving the context. Every completion closes the tracked connection and notifies the metrics collector (`TransactionCompleted`) so telemetry stays accurate.
...
### The client-side dispose guarantee has a server-side backstop, verified empirically

The dispose-triggers-rollback contract above only covers the case where `Dispose`/`DisposeAsync` actually runs. If the connection dies before that — process crash, killed session, severed network — pengdows.crud's own code never gets a chance to run at all, so the guarantee that matters there belongs to the *database engine*, not to `TransactionContext`. No RDBMS commits a transaction on disconnect; an in-flight transaction whose connection dies before `COMMIT` is sent is rolled back by the server itself. This isn't just documented behavior — `testbed.TestProvider.TestTransactionRollbackOnKilledConnection` (overridden in `SqlServerTestProvider`, `PostgreSQLTestProvider`, `MariaDbTestProvider`) proves it directly against real servers: open a raw ADO.NET connection (bypassing pengdows.crud entirely), start a transaction, insert a row, then kill that connection's server-side session from a second connection (`KILL <spid>` on SQL Server, `pg_terminate_backend(pid)` on PostgreSQL, `KILL CONNECTION <id>` on MariaDB) — without ever calling `Commit()`, `Rollback()`, or even a clean `Dispose()` on the first connection. A third connection then confirms the row never became visible. Verified passing across SQL Server 2017/2019/2022, PostgreSQL 9.5/15/16, and MariaDB 10.2/10.4/10.11/11.4. Embedded/in-process engines (SQLite, DuckDB) have no separate server process to kill — for those, the client-side dispose path covered above is the only mechanism, and `SqliteTestProvider`'s `TestSingleConnectionModeOrdinaryCommandIsolation`/`TestSingleWriterModeConcurrentWriteTransactionSerialization` already cover it.
...
// ...or, if that work succeeded and you no longer need the savepoint (only on dialects whose
// SavepointCapabilities includes Release — not SQL Server/Sybase/Oracle):
