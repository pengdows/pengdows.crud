...
- **Identifier quoting** — `WrapObjectName()` calls (used to quote table/column names) run once, at
  *build* time, before the query text ever reaches `Clone`. Their output is already baked into the
  ...
  reuses whatever quoting characters the original build produced. Every shipped dialect except
  Access uses the ANSI double-quote (`"name"`) policy (`SqlDialect.QuotePrefix`/`QuoteSuffix`
  defaults), so cloning among them is safe. **Access is the exception:** `AccessDialect` quotes
  with `[brackets]` (Jet/ACE rejects ANSI double-quoted identifiers), so a container built against
  an Access context and cloned to any other dialect — or vice versa — carries the wrong quote
  characters. Build a separate template per quoting style if you mix Access with other databases.
