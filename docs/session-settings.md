...
- Persistent modes reuse the same tracked connection and use `_sessionSettingsAppliedOnOpen` plus reference checks to avoid applying settings twice.
...
If anyone changes `DatabaseContext` to touch `_sessionSettingsDetectionCompleted`, `_sessionSettingsAppliedOnOpen`, or `ApplyConnectionSessionSettings`, rerun these tests and make sure this document still reflects the story before merging.
