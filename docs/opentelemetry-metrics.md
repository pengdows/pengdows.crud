...
times out — see `docs/connection-management-guide.md`'s "Shared connection locking &
timeouts" section. An earlier draft of *this*
...
- `pool.label` (`reader`/`writer`) — additionally applied to pool-governor instruments only
  ...
  by read/write role, only by context. This is a real, still-open gap (the OTel bridge
  doesn't yet expose the `DatabaseMetrics.Read`/`.Write` per-role split that
  `IDatabaseContext.GetPoolStatisticsSnapshot(PoolLabel)` already has internally).
...
The OTel semantic-convention pool-side
