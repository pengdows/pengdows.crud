...
## Pool governors
The context creates read and write `PoolGovernor` instances in every mode except `SingleConnection`, issuing `PoolSlot` tokens before any connection is acquired. Each governor waits no longer than `PoolAcquireTimeout` (default 10 seconds via `DatabaseContextConfiguration.PoolAcquireTimeout` — deliberately below the ~15 s "Connect Timeout"/"Connection Timeout" default shared by SqlClient, Npgsql, MySqlConnector, and Oracle's managed driver, so pengdows' own `PoolSaturatedException` reliably surfaces before the provider's own connection-open timeout would) before throwing a `PoolSaturatedException` with queue and slot statistics, so you fail fast instead of saturating the provider pool. Override `MaxConcurrentReads`/`MaxConcurrentWrites` to tune effective limits, and observe the snapshots if you need to correlate hot paths with pool contention.
...
**The default formula is count-based, not duration-aware — size `MaxQueuedWrites` deliberately for `SingleWriter`, don't just trust the default.** `Math.Max(effectiveSlots × 8, 32)` bounds *how many* callers may queue, but says nothing about *how long* a queued caller can expect to wait, and for `SingleWriter` specifically those two things are only loosely related: a writer holds the turnstile for its **entire transaction**, not for one statement, so the real per-waiter wait time is `(queue position) × (typical transaction duration)`, not a function of slot count at all. A workload with many short writers per transaction (the common case) drains its queue quickly and can safely run with a much deeper queue than the default 32 without ever approaching `PoolAcquireTimeout`; a workload with few, long-running writer transactions can exhaust a much shallower queue's realistic wait budget. The default cap does not know which situation it's protecting — it was sized as a generic circuit breaker, not tuned per-`DbMode`. If your write transactions are short relative to `PoolAcquireTimeout`, raise `MaxQueuedWrites` explicitly to the concurrency level you actually expect rather than accepting fast rejections at the default's count; if they're long, the default's fast-rejection behavior is probably closer to what you want already. There is currently no built-in mechanism that estimates expected wait from tracked hold-time statistics and admits or rejects accordingly — see `docs/planning/future-work.md`'s FEAT-018 entry for that as a possible future direction.
