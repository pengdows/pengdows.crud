...
**PreventDatabaseUnload exists for exactly one reason, and it has nothing to do with performance.** It is not a health check, TCP keepalive, periodic ping, warmed working connection, or concurrency mode. It retains a single passive sentinel connection solely to prevent a target database from unloading, deactivating, closing, or pausing.
...
The mode retains exactly one sentinel connection: the connection opened during construction for dialect detection is kept open (`DatabaseContext.PersistentConnection`) instead of being disposed. **The sentinel never executes application commands, opens transactions, or hands work to callers** — every real read and write still goes through its own fresh ephemeral connection exactly like Standard mode — and, because it is never used for work, session settings are not applied to it. The sentinel is attached to a pinned governor slot, so effective working capacity is the configured capacity minus the retained sentinel.
...
- There is a single sentinel per context, even when separate read and write connection strings are configured. The sentinel is not health-checked or replaced after construction (see §6).
...
- **After construction, neither persistent mode recreates its long-lived connection.** For `PreventDatabaseUnload` this only affects the unload protection — the sentinel is never used for work, so a dropped sentinel does not break reads or writes, which keep using their own ephemeral connections; the protection is simply lost until the context is recreated. `SingleConnection` cannot safely recreate a connection for disposable `:memory:` databases; a replacement would be a different empty database.
