...
**DI key of a keyed `DbProviderFactory` registration**, not necessarily an ADO.NET invariant
name: `TenantContextRegistry` resolves each tenant's factory with
`GetKeyedService<DbProviderFactory>(config.ProviderName)`, so every `ProviderName` your tenants use
needs a matching `services.AddKeyedSingleton<DbProviderFactory>("sqlserver", SqlClientFactory.Instance)`
(etc.) registration — `AddMultiTenancy` does not register provider factories for you. See
[`dynamic-provider-loading.md`](./dynamic-provider-loading.md#di-registration-key-vs-providername--the-gotcha)
for the same contract as it applies to `DbProviderLoader`.
...
- `ITenantContextRegistry` as a singleton (`TenantContextRegistry`), wired to the resolver and
  factory above. The registry is constructed by the container, so its optional `maxTenantCount`
  argument is left at its default (`null`, unbounded) — see "Tenant cardinality" below for how to
  set a cap
...
```csharp
// 1. A keyed DbProviderFactory for every ADO.NET provider your tenants use — the same
//    requirement as the configuration-driven path.
...
//    TenantContextRegistry's constructor also takes IServiceProvider and ILoggerFactory. The
//    container supplies IServiceProvider itself; ILoggerFactory comes from AddLogging() (already
//    called for you by the ASP.NET Core / Generic Host builders).
...
`GetContext` returns a bare `IDatabaseContext` reference with no protection
...
completes atomically with respect to a concurrent rotation. `AcquireLease` closes that gap:
...
`GetContext` remains the right default.
...
code path that holds a tenant context across an `await` uses `AcquireLease` (see above) rather
than caching a bare `GetContext` reference.
...
drain wait — is dispatched onto the thread pool rather than run inline, so the invalidating or
lease-releasing thread never becomes a synchronous-blocking passenger on that drain. Don't assume a tenant's context is already disposed the instant `Invalidate`/lease-`Dispose`
returns — if you need to observe completion, subscribe to `ContextRemoved`.

**The one residual gap, by design, not oversight:** `GetContext`'s bare-reference callers still
have no protection against a concurrent `Invalidate` disposing the exact context they just
received — including one whose construction was still in flight when `Invalidate` ran (the
registry disposes it once construction completes, but the racing `GetContext` caller is still
handed that reference). That's precisely what `AcquireLease` exists to close. For the
...
- **Pure SQL-text templates** (`_templatesByDialect`) — also keyed by **dialect instance**
  (`ConditionalWeakTable<ISqlDialect, Lazy<CachedSqlTemplates>>`), so differently-versioned servers
  of the same product never share stale SQL. The trade-off: many tenants standardized on the
  identical engine+version each hold their own copy of the templates (one per live context), rather
  than sharing a single entry; each copy is reclaimed with its dialect.
...
The comment block above these fields in `TableGateway.Core.cs` (search `_insertBinders`) states the actual bug
...
Because dialect-derived caches are keyed by dialect instance (not by tenant identity),
...
`TenantContextRegistry` accepts an optional `maxTenantCount` constructor argument. Once that many
distinct tenants are cached, `GetContext`/`AcquireLease` for a *new* tenant throws
`InvalidOperationException` (call `Invalidate`/`InvalidateAll` to free capacity); already-cached
tenants keep resolving normally. Admission is atomic — the count check and the cache insert happen
under one lock, so concurrent first requests for different new tenants cannot overshoot the cap.

`MultiTenantOptions` has no cardinality setting and `AddMultiTenancy` does not pass one, so the
registry it registers is unbounded (the default). To set a cap, register the registry yourself
with a factory delegate — after `AddMultiTenancy` (the last `ITenantContextRegistry` registration
is the one the container resolves) or in a fully custom setup:
...
Set a cap for any long-lived process serving many distinct, dynamically discovered tenants, to
bound worst-case connection-pool growth.
