...
`ISqlDialect.AnalyzeException(Exception)` (implemented by `SqlDialect.AnalyzeException` in `pengdows.crud/dialects/SqlDialect.cs`) returns:
...
(335544665 unique, 335544466 foreign key — both directions — 335544347 not-null, 335544558
check). Unlike
...
anything itself, and pengdows.crud ships no built-in retry coordinator. Retry policy is an
application concern. The typed exceptions pengdows.crud throws also carry
`DatabaseException.IsTransient` (set by the exception translators, independently of
`AnalyzeException`), which is the natural signal to key a retry off when you catch the typed
exception rather than the raw provider one.

A minimal hand-rolled application-level wrapper around `IsRetryable` looks like this. Keep
the retried unit small and idempotent, and don't hold a transaction open across the backoff
delay:
