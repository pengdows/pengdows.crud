...
- Batch update uses dialect-specific SQL only when the dialect advertises `SupportsBatchUpdate`; otherwise it falls back to one update statement per entity. As of this writing, `SupportsBatchUpdate` is `true` for PostgreSQL/CockroachDB/YugabyteDB (`UPDATE ... FROM (VALUES ...)`), SQL Server (`MERGE ... USING (VALUES ...)`), and Snowflake (`UPDATE ... FROM (VALUES ...)`, no target alias). All other dialects — including Oracle — fall back to one update per entity. `PrimaryKeyTableGateway<TEntity>` batch update always builds one update container per entity.
...
None of these are part of the current public API. Provider-native bulk loaders/pipelines in
particular (`SqlBulkCopy`/`COPY`/`MySqlBulkCopy`/DuckDB's `Appender`) aren't semantically uniform
across providers, so a caller-facing API built on them would break this library's
