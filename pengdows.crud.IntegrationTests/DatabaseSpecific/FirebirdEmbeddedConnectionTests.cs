...
/// database through its embedded engine in the current process. The engine is provisioned by
/// scripts/install-firebird-embedded.sh (run-integration-tests.sh does this automatically). A
/// missing engine fails the tests instead of skipping them: an unprovisioned environment is not
/// a database capability limit.
/// </summary>
public sealed class FirebirdEmbeddedConnectionTests
{
    ...
    private static async Task CreateDatabaseAsync(string path)
    {
        ...
        {
            throw new InvalidOperationException($"Firebird Embedded is unavailable: {ex.Message}");
        }
    }
