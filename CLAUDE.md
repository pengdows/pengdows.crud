...
15. **`pengdows/internal/DatabaseDetectionService.cs` has TWO independent live-probe implementations that must each get the new database's discriminator probe: `DetectFlavorWithDetailAsync` (used by `DataSourceInformation.CreateAsync`) and its synchronous twin `DetectFlavorWithDetail` (used by `DatabaseContext`'s ordinary, synchronous constructor — the path essentially every real caller goes through).** Adding a probe to only one of them compiles clean, passes any fakeDb-based unit test that happens to exercise only the async path, and *still fails silently at runtime* — the sync path just falls through and misclassifies the new database as whatever generic family it wire-shares with (e.g. plain PostgreSQL for a Postgres-wire-compatible database), so the new `<Name>Dialect`'s own overrides never actually apply for a normal `new DatabaseContext(...)` call. This is exactly what happened with Spanner: the async twin got a `SHOW SPANNER.OPTIMIZER_VERSION` discriminator probe, the sync twin didn't, and every real Spanner connection built the normal way silently ran as plain `PostgreSqlDialect` instead of `SpannerDialect` — undetected because nothing had ever unit-tested the sync path's Spanner detection at all. Add the identical probe to both, and add a unit test for both (see `pengdows.crud.Tests/internal/DatabaseDetectionFlavorTests.cs`'s `DetectProduct_Identifies*` tests for the pattern — they exercise `DatabaseDetectionService.DetectProduct`, i.e. the sync path).
