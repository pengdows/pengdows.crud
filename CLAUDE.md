...
| `PreventDatabaseUnload` | 1 | **Production-supported lifecycle mode** — Standard connection-per-operation behavior plus one unused permit-backed sentinel per materially separate pool; auto-selected by `Best` for SQL Server LocalDB and Firebird (embedded or ordinary client-server — see below) |
...
**Standard, PreventDatabaseUnload, and SingleWriter are all production-supported modes**, each for a different deployment shape: Standard for client-server databases; PreventDatabaseUnload for databases whose lifecycle benefits from a passive attachment, including SQL Server LocalDB and Firebird; SingleWriter for file-based SQLite/DuckDB, with turnstile-governed write serialization. PreventDatabaseUnload sentinels never run application work, and each consumes a permit from its corresponding pool.

**Firebird's `Best` resolution applies uniformly to embedded AND ordinary client-server connections — this is not an embedded-only quirk.** Under Firebird's default SuperServer architecture, `RDB$LINGER` defaults to `0`/`NULL`: the moment the last attachment to a database closes, the engine closes that database's file and discards its page cache immediately (terminology note: Firebird does not unload the *server* — the server process keeps running; only that one database's cache/attachment state unloads). `Linger` applies only to SuperServer, not Classic/SuperClassic, but the client side can't reliably tell which server architecture it's talking to, so the protective default applies uniformly. This is empirically verified, not just documented: `testbed`'s `TestIdleUnloadProbe` sets `ALTER DATABASE SET LINGER TO 1`, drains the connection pool, and measures a real cold-vs-warm round-trip latency difference against a live container (consistently ~5-10x across Firebird 3.0/4.0/5.0 in CI) — see `TryEnableFastIdleUnloadAsync`/`ClearProviderPoolForIdleUnloadProbe` in `testbed/TestProvider.cs` and their Firebird overrides. `Best` is the only resolution Firebird coerces; an explicit `Standard` (or any other mode) request is genuinely safe and honored, unlike LocalDB below. An application that would rather manage this at the database level directly can do so via `ALTER DATABASE SET LINGER TO n`, independent of pengdows.crud's own mode. **Do not casually extend this same treatment to another database without the same empirical bar** — a documentation claim or a chat-message table is not sufficient evidence to change a shipped dialect's default mode resolution; a Db2 change was proposed and reverted in the same session specifically because it had no such verification, before being properly checked afterward. Two more databases have since been run through the same probe, with opposite, informative results: **Db2** — `TryEnableFastIdleUnloadAsync` returns `true` unconditionally (Db2's implicit-activation deactivation is immediate by default, no delay to configure) and `ClearProviderPoolForIdleUnloadProbe` calls `DB2Connection.ReleaseObjectPool()`; the measured cold/warm gap was only ~2ms (noise, below the probe's 5ms floor) — **confirms Db2 is genuinely fine in `Standard`, `Best` stays unchanged**. **SQL Server with `AUTO_CLOSE` explicitly turned on** (off by default, so this doesn't change SQL Server's own `Best` resolution) — `TryEnableFastIdleUnloadAsync` runs `ALTER DATABASE CURRENT SET AUTO_CLOSE ON` and `ClearProviderPoolForIdleUnloadProbe` calls `SqlConnection.ClearAllPools()`; the measured cold/warm ratio was ~68x (cold ≈ 40ms) across SQL Server 2017/2019/2022 — a real, larger-than-Firebird's reconnect cost confirming that an application running with `AUTO_CLOSE ON` should explicitly request `PreventDatabaseUnload` itself (already an honored explicit choice for ordinary SQL Server today).
