...
**`PreventDatabaseUnload` auto-selection is deliberately narrow: LocalDB only.** Several databases have a real, empirically-confirmed idle-triggered reconnect/reactivation cost — Firebird SuperServer's default `RDB$LINGER=0` (page cache discarded immediately after the last attachment closes — terminology note: Firebird doesn't unload the *server*, the process keeps running; only that one database's cache/attachment state unloads), Db2 LUW's implicit-activation lifecycle, SQL Server with `AUTO_CLOSE` turned on. `testbed.TestProvider.TestIdleUnloadProbe` measures this directly against live containers rather than trusting any claim (see `TryEnableFastIdleUnloadAsync`/`ClearProviderPoolForIdleUnloadProbe` and each database's overrides) — Firebird showed a consistent ~5-10x cold/warm ratio across 3.0/4.0/5.0, and a follow-up sentinel-validation step confirmed `PreventDatabaseUnload`'s actual mechanism (one connection held open) genuinely closes that gap for at least one confirmed case (SQL Server `AUTO_CLOSE`, ~68x ratio collapsed to near-warm). But **detecting a real cost does not mean `Best` should auto-select `PreventDatabaseUnload` for that database** — two considerations cut the other way: a heavily-trafficked deployment may never actually drain its connection pool to zero, so the cost never materializes while the sentinel's permit is still paid unconditionally; and a deployment deliberately built to scale-to-zero for cost reasons (the Firebird/Db2/SQL-Server case is architectural and unwanted by everyone, but genuinely cost-optimized serverless products like Azure SQL serverless or Aurora Serverless are a different animal entirely — see `docs/connection/connection-modes.md`) would have that intentional behavior silently defeated by a forced sentinel. Only the operator knows which situation applies to their deployment, so for every one of these confirmed-real-cost databases, `PreventDatabaseUnload` stays a fully-supported, explicitly-honored **knob** — never an auto-selected default — and `Best` resolves to `Standard` exactly like any other full-server database. LocalDB is the sole exception: there is no production LocalDB deployment shape where the auto-shutdown behavior is wanted, so it stays an unconditional `Best` selection, and every other explicitly-requested mode still forces `PreventDatabaseUnload` too — **except an explicit `Standard` request, which is honored.** The sentinel is only useful to a workload that actually goes idle long enough to trigger LocalDB's auto-shutdown; a caller whose workload stays busy continuously can opt out deliberately (`SqlServerDialect.CoerceConnectionMode`), and gets a performance-only (not correctness-risk) warning logged when they do.

**DuckDB and Access similarly allow an explicit `Standard` request to be honored, unlike SQLite.** Both engines are documented by their own vendors as supporting concurrent connections/writers — `Best` still resolves to `SingleWriter` for both, and `PreventDatabaseUnload` still coerces to `SingleWriter` too, but `Standard` specifically is let through (`SqlDialect.CoerceEmbeddedSingleWriterMode`'s `allowStandard` parameter; SQLite never sets it and stays hard-coerced with no opt-out). `DatabaseContext.WarnOnModeMismatch` logs an evidence-backed risk warning instead of silently trusting that vendor documentation: **both are now CONFIRMED LIVE, not just architecturally reasoned.** DuckDB's vendor claim genuinely holds for disjoint-row concurrent writers (they don't conflict, matching the documentation), but 20 concurrent writers updating the *same* row through pengdows.crud under `DbMode.Standard` threw `SerializationConflictException` (`"TransactionContext Error: Conflict on update!"`) for 8/20 — the identical scenario under `DbMode.SingleWriter` (the default) succeeded 20/20 with zero conflicts. See `pengdows.crud.IntegrationTests/ErrorHandling/SerializationConflictTests.cs` for the checked-in classification proof.

**Access's `Standard`-mode finding was re-verified end-to-end on a real Windows machine (2026-09-18) and turned out to rest on a real, separate bug, not just an unverified risk.** The original "raw concurrent OleDb writers hit `OleDbException: Could not update; currently locked.`" claim was accurate but came from an ad hoc, never-committed script. Re-verifying it surfaced a connection-string bug that made `DbMode.Standard` fail outright — not just under contention, but on *every* connection open, including a single non-concurrent `CREATE TABLE`: `AccessDialect` never overrode `SupportsExternalPooling`/`PoolingSettingName`, so pengdows.crud was injecting an ADO.NET-style `Pooling=True` into the OLE DB connection string, and Jet/ACE throws the generic `"Could not find installable ISAM"` for *any* unrecognized connection property (confirmed: `Application Name` fails identically). Fixed by overriding both to `false`/`null` (architecturally identical to DuckDB's "in-process, no external pooling switch" reasoning). Note: the original justification for this fix claimed `System.Data.OleDb` "pools transparently by default anyway" — that claim was challenged and retracted after a dedicated discriminating test (same connection string repeated vs. a fresh never-seen string each time) came back statistically indistinguishable, and disabling native pooling services outright made opens marginally *faster*, not slower. The honest reasoning: a local file attach has no network handshake to amortize, so pooling isn't a meaningful concept here at all — not that it's silently already happening. The fix itself is unaffected; only the stated reasoning was wrong. See `AccessDialect.cs`'s file-level AI SUMMARY for the full correction. Once fixed, a naive re-test (bare auto-committing `INSERT`s) misleadingly showed 20/20 success under `Standard` — a single-statement write doesn't hold its lock long enough to collide even under real concurrency. Redone fairly, with the same held-open-transaction contention pattern the original claim used (via `Context.BeginTransaction()`, through the real public API, not raw OleDb): `DbMode.Standard` reproduced the hazard clearly — 14-18/20 failures (`CommandTimeoutException`, `"Could not update; currently locked."`) — while `DbMode.SingleWriter` stayed at 0/20 across every run. The conflict was also confirmed NOT table-scoped (disjoint writes across two different tables in the same file still collided).

**This makes Access's `Standard`-mode risk categorically worse than DuckDB's, not just "also risky" — the two are not equivalent, and the wording must not imply they are.** DuckDB's `Standard` risk has a genuine, confirmed safe zone: its optimistic concurrency control lets disjoint-row writers through cleanly (matching the vendor's documentation), and the ONLY confirmed failure is same-row/same-resource contention — so "avoid concurrent writers touching the same row" is real, actionable, safe advice for DuckDB under `Standard`. Access has no equivalent safe zone: the conflict spans disjoint rows in the same table AND disjoint rows across different tables in the same file. There is no "just don't touch the same row" escape hatch for Access the way there is for DuckDB — a caller being "adventurous" with `Standard` on DuckDB has a real, working middle ground; the same caller being adventurous with `Standard` on Access does not. `AccessDialect.DescribeStandardModeRisk()` states this explicitly rather than only listing failure numbers, specifically so a reader who already knows DuckDB's safe-zone framing doesn't wrongly assume it transfers. A related gap found and fixed the same session: `AccessDialect` had no `ReadOnlyPoolDiscriminatorSettingName`, so its reader/writer connection strings were identical and shared one physical `OleDb` pool — fixed via `"Jet OLEDB:Database Locking Mode"` => `"1"` (confirmed both ACE-recognized and behaviorally inert, matching ACE's own documented default), mirroring `OracleDialect`'s identical fix for ODP.NET. See `AccessDialect.cs`'s file-level AI SUMMARY and `docs/connection/access-concurrency-verification.md` for the full trail, and each dialect's `DescribeStandardModeRisk()` override for the exact wording surfaced to callers. **Bottom line: `SingleWriter` is the only mode confirmed both correct and fully concurrent for Access** — `SingleConnection` would also be correctness-safe but serializes all access (including reads), so it isn't a practical alternative.
